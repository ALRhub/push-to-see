...
class Trainer(object):
    ...
    def preload(self, transitions_directory):
        ...
        self.loss_log = np.loadtxt(os.path.join(transitions_directory, 'loss.log.txt'), delimiter=' ')
        self.loss_log.shape = (self.loss_log.shape[0],1)
        self.loss_log = self.loss_log.tolist()
    ...
    def get_reward_value(self, next_color_heightmap, next_depth_heightmap, prev_seg_score, curr_seg_score, change_detected):
        ...
        if self.method == 'reactive':
            ...
            # if primitive_action == 'push':
            # if change_detected:
                # evaluate rewards intrinsically with Q values with a forward pass below
            # if (curr_seg_score - prev_seg_score) == 0:
            #     curr_seg_rew = curr_seg_score
            # else:

            # #this should be in the actual training
            # if sess_success and sess_iteration < 10:
            #     current_reward = 10
            # elif sess_success:
            #     current_reward = 5
            # else:
            #     if (curr_seg_score - prev_seg_score) > 0.4:
            #         current_reward = 1.0
            #     elif (curr_seg_score - prev_seg_score) > 0.01:
            #         current_reward = 0.3
            #     elif (curr_seg_score - prev_seg_score) < -0.4:
            #         current_reward = -0.5
            #     elif (curr_seg_score - prev_seg_score) < -0.01:
            #         current_reward = -0.1
            #     else: #(-0.01 to 0.01)
            ...
            if change_detected:
                ...
                # current_reward = np.abs(diff) * np.exp(np.abs(diff))
                if diff < 0:
                    current_reward *= -1
            else:
                current_reward = -0.5
            ...
            print('------------------------------- Push-to-See -------------------------------')
            print('Current segmentation score --> ', curr_seg_score, ' Previous segmentation score --> ', prev_seg_score)
            print('Segmentation difference after pushing --> ' + BColors.WARNING, curr_seg_score - prev_seg_score, BColors.ENDC)
            print('CURRENT REWARD --> ' + BColors.WARNING + '{:.5f}'.format(current_reward) + BColors.ENDC)
            ...
            #     # future reward discount --> 0.5 --> default val
    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        ...
        if self.method == 'reactive':
            ...
            if primitive_action == 'push':
                ...
                # push_predictions, grasp_predictions, state_feat
                push_predictions, grasp_predictions = self.forward(color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=best_pix_ind[0])
            ...
            # Do forward pass with specified rotation (to save gradients)
            # push_predictions, grasp_predictions, state_feat
            push_predictions = self.forward(color_heightmap, depth_heightmap, is_volatile=False,
            ...
            loss_value = loss.cpu().data.numpy()
            l_value = loss_value
            print(BColors.OKBLUE + 'Training loss: %f' % (loss_value) + BColors.OKBLUE)
            self.optimizer.step()
